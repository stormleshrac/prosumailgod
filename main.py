...
listproxy = []
#urlist = []
msgconf = "✔️ Configuración Guardada"
...
def nameRamdom():
    ...
    name = "".join(random.sample(populaton,6))
    return name
...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if data:
        ...
        bot.sendMessage(-1001299122115,'😎 DOWNLOAD BY 😎\n 📶 @'+username+' ⬇⬆')
        bot.sendMessage(-1001299122115,finishInfo+'\n'+filesInfo,parse_mode='html')
        if len(files)>0:
...
def sendTxt(name,files,update,bot):
                ...
                bot.sendFile(-1001299122115,name)
                os.unlink(name)
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:
            ...
            mensaje = "🚷 No tienes Acceso, \n Favor de Contactar con mi programador 🚷"
            reply_markup = inlineKeyboardMarkup(
                r1=[inlineKeyboardButton('⚙ Programador ⚙',url='https://t.me/raydel0307')]
            )
            msg1= "🚷 ALERTA, Usuario no identificado 🚷"
            rep_markup = inlineKeyboardMarkup(
                r1=[inlineKeyboardButton('📞 Contactar',url=f'https://t.me/{username}')]
            )
            bot.sendMessage(-1001299122115,msg1,reply_markup=rep_markup)
        ...
        if '/aggproxy' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /aggproxy proxy❌')
            else:
                bot.sendMessage(update.message.chat.id,'❌No Tiene Permiso❌')
            return
        if '/checkproxy' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMessage(update.message.chat.id,'❌No Tiene Permiso❌')
            return
       # if '/sendtxt' in msgText:
           # name = str(msgText).split(' ')[1] + '.txt'
           # for url in urlist:
              #  urls = url + '\n'
              #  txt = open(name,'w')
              #  txt.write(urls)
        ...
        if '/shorturl' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,msgconf,reply_markup=reply_markup)
                except:
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /shorturl ❌')
            return
        ...
        if '/rename_on' in msgText:
            try:
                ...
                if getUser:
                    ...
                    bot.sendMessage(update.message.chat.id,'Los archivos serán renombrados')
            except:
                bot.sendMessage(update.message.chat.id,'Error en el comando /rename_on')
            return
        if '/rename_off' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,'Los archivos mantendrán su nombre original')
            except:
                bot.sendMessage(update.message.chat.id,'Error en el comando /rename_off')
            return	
        if '/ban' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
        ...
        if '/uptype' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,msgconf)
            except:
        ...
        if '/enc' in msgText:
            ...
            bot.sendMessage(update.message.chat.id, f'/proxy socks5://{proxy}')
            return
        if '/dec' in msgText:
            proxy_sms = str(msgText).split(' ')[1]
        ...
        if '/xyz' in msgText:
            global urlist
            text_sms = str(msgText).split(' ')[1]
            enc_text = cryptocode.encrypt(text_sms,'R@ydel2022')
            bot.sendMessage(update.message.chat.id, f'{enc_text}\n')
            #urlist.append(str(enc_text))
            #counter += 1
            return
        if '/zyx' in msgText:
            text_sms = str(msgText).split(' ')[1]
            dec_text = cryptocode.decrypt(text_sms,'R@ydel2022')
            bot.sendMessage(update.message.chat.id, f'{dec_text}\n')
            return
        if '/dir' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            reply_markup = inlineKeyboardMarkup(
                r1=[inlineKeyboardButton('📊 Canal de Descargas 📊', url='https://t.me/descargasultimatedownload'),
                    inlineKeyboardButton('⚙ Soporte ⚙', url='https://t.me/raydel0307')]
            )
            ...
            if loged:
                ...
                bot.editMessageText(message,'❌Error y Causas🧐\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
        elif '/clear' in msgText and user_info['cloudtype']=='moodle':
            contador = 0
            eliminados = 0
...
def main():
    #bot_token = os.environ.get('bot_token')
    #.sendMessage(-1001299122115,'❗ Bot Reiniciado ❗')
    #set in debug
    bot_token = '5613702975:AAHg171JYIa4w0TX_1ubK82VGWIdh1OAwBM'
